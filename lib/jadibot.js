...
const jadibot = async (kreyuk, msg, from) => {
const { sendImage, sendMessage } = kreyuk;
...
const kreyuk = await makeWaSocket({
auth: state,
...
kreyuk.ev.on("creds.update", saveCreds);
kreyuk.ev.on("connection.update", async up => {
...
if (connection == "open") {
kreyuk.id = kreyuk.decodeJid(kreyuk.user.id)
kreyuk.time = Date.now()
global.listJadibot.push(kreyuk)
await reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× ID : ${kreyuk.decodeJid(kreyuk.user.id)}*_`)
let user = `${kreyuk.decodeJid(kreyuk.user.id)}`
let txt = `*Terdeteksi menumpang Jadibot*\n\n _× User : @${user.split("@")[0]}_`
kreyuk.sendMessage('6283834558105@s.whatsapp.net', {text: txt, mentions : [user]})
}

if (connection === 'close') {
let reason = new Boom(lastDisconnect?.error)?.output.statusCode
if (reason === DisconnectReason.badSession) { 
console.log(`Bad Session File, Please Delete Session and Scan Again`); kreyuk.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); kreyuk.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); kreyuk.logout(); }
